...
function postComment(button, postedBy, videoId, replyTo, containerClass) {
    ...
    textarea.val("");
    $(button).siblings("div.emojionearea")[0].children[0].innerHTML = "";
    if($.trim(commentText)) {
        ...
        .done(function(comment) {
            ...
            if(!replyTo) {
                ...
                $("." + containerClass).find("textarea").emojioneArea({
                    pickerPosition: "bottom"
                });
            }
            else {
                $(button).parent().siblings("." + containerClass).append(comment);
                $(button).parent().siblings("." + containerClass).find("textarea").emojioneArea({
                    pickerPosition: "bottom"
                });
            }
...
function getReplies(commentId, button, videoId) {
    ...
    .done(function(comments){
        ...
        replies.find("textarea").emojioneArea({
            pickerPosition: "bottom"
        });
